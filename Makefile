...
# on a 1024 canvas (Apple's grid, SUITE.md), which fills 80.5% of the tile --
...
icons:
	...
	@# the margin-cropped canvas (SUITE.md, 2026-09-18). The .icns and the
...
$(TAURI_BIN): $(shell find src src-tauri/src src-tauri/icons -type f) package.json src-tauri/Cargo.toml src-tauri/tauri.conf.json tailwind.config.ts postcss.config.js vite.config.ts index.html
...
install: install-guard $(TAURI_BIN)
	...
	sed '1s|viewBox="[^"]*"|viewBox="$(LINUX_VIEWBOX)"|' icon.svg > $(ICONDIR)/ndisc.svg
	chmod 0644 $(ICONDIR)/ndisc.svg
