#!/bin/bash
# Build ndisc and install it to /Applications, then relaunch. macOS only.
#
...
echo "--- Quitting running ndisc (if any) ---"
osascript -e 'quit app "ndisc"' 2>/dev/null || pkill -x ndisc 2>/dev/null || true
sleep 1
...
echo "The Keychain trusts the binary that created an entry, so a rebuilt ndisc"
