...
// Vite-emitted URLs (`/assets/...`) in localStorage. Those paths 404 once the
// bundle is removed, so blank the URL — the entry stays so the user can see
// which labels need a fresh nostr.build upload.
export function clearStaleBundleUrls(entries: LabelEntry[]): LabelEntry[] {
  let changed = false;
  const next = entries.map((l) => {
    if (l.imageUrl.startsWith("/assets/")) {
      changed = true;
      return { ...l, imageUrl: "" };
    }
    return l;
